export type Lang = 'en' | 'fr';

export const translations = {
  // ===== NAV =====
  'nav.home': { en: 'Home', fr: 'Accueil' },
  'nav.services': { en: 'Services', fr: 'Services' },
  'nav.about': { en: 'About', fr: 'À propos' },
  'nav.caseStudies': { en: 'Case Studies', fr: 'Études de cas' },
  'nav.contact': { en: 'Contact', fr: 'Contact' },
  'nav.legal': { en: 'Legal', fr: 'Mentions légales' },
  'nav.cta': { en: 'Book a Consultation', fr: 'Réserver' },

  // ===== HERO =====
  'hero.badge': {
    en: 'Independent Contractor · Full-Stack Software Engineer · Available globally',
    fr: 'Independent Contractor · Full-Stack Software Engineer · Disponible à l\'international',
  },
  'hero.title1': { en: 'Full-Stack Software Engineer', fr: 'Full-Stack Software Engineer' },
  'hero.title2': { en: 'AI Systems · Digital Content Platforms', fr: 'AI Systems · Digital Content Platforms' },
  'hero.subtitle': {
    en: 'I build full-stack software for international clients: type-safe front-ends, Python and Node back-ends, API design, infrastructure and the AI systems and Digital Content Development platforms that ship Digital Creative Content to real users. Production-grade code, full ownership transferred at delivery.',
    fr: "Je conçois du software full-stack pour des clients internationaux : front-ends typés, back-ends Python et Node, design d'API, infrastructure, et les systèmes IA et plateformes de Digital Content Development qui livrent du Digital Creative Content à de vrais utilisateurs. Code production-grade, transfert de propriété intégral à la livraison.",
  },
  'hero.cta.primary': { en: 'Book a Consultation', fr: 'Réserver une consultation' },
  'hero.cta.secondary': { en: 'Explore Services', fr: 'Découvrir les services' },

  'stats.years': { en: 'Years in Tech', fr: 'Années en tech' },
  'stats.workflows': { en: 'AI Workflows Deployed', fr: 'Workflows IA déployés' },
  'stats.clients': { en: 'International Clients', fr: 'Clients internationaux' },
  'stats.tools': { en: 'Custom Software Tools', fr: 'Logiciels sur-mesure' },

  // ===== SERVICES =====
  'services.kicker': { en: '// Services', fr: '// Services' },
  'services.title': {
    en: 'Three full-stack engagements, built for production.',
    fr: 'Trois missions full-stack, conçues pour la production.',
  },
  'services.subtitle': {
    en: 'Each service is delivered as an Independent Contractor under a formal scope of work, with defined deliverables and a transparent transfer of ownership at close-out.',
    fr: "Chaque service est délivré en tant qu'Independent Contractor sous périmètre formel, avec livrables définis et transfert de propriété transparent en fin de mission.",
  },
  'services.viewAll': { en: 'View all services', fr: 'Voir tous les services' },

  'service.ai.title': { en: 'AI Optimization & Prompt Engineering', fr: 'AI Optimization & Prompt Engineering' },
  'service.ai.desc': {
    en: 'Full-stack integration of Artificial Intelligence into your product: typed prompt pipelines, retrieval-augmented systems, API design, observability and CI/CD. I ship LLM features that survive production traffic — not demo prototypes.',
    fr: "Intégration full-stack d'Intelligence Artificielle dans votre produit : pipelines de prompts typés, systèmes RAG, design d'API, observabilité et CI/CD. Je livre des fonctionnalités LLM qui survivent au trafic de production — pas des prototypes de démo.",
  },
  'service.content.title': { en: 'Digital Content Development', fr: 'Digital Content Development' },
  'service.content.desc': {
    en: 'Full-stack software behind your Digital Content Development: interactive web apps, custom CMSes, structured content data models, production pipelines and Digital Creative Content delivery. Back-end to front-end, you own the source code.',
    fr: "Le software full-stack derrière votre Digital Content Development : web apps interactives, CMS sur-mesure, modèles de données structurés, pipelines de production et livraison de Digital Creative Content. Back-end et front-end, le code source vous appartient.",
  },
  'service.logic.title': { en: 'Computational Logic & Algorithm Consulting', fr: 'Computational Logic & Algorithm Consulting' },
  'service.logic.desc': {
    en: 'Algorithmic engineering for software teams: I translate ambiguous product questions into formal specifications, reference implementations in Python or TypeScript, complexity analysis and measurable success criteria.',
    fr: "Ingénierie algorithmique pour équipes software : je transforme des questions produit ambiguës en spécifications formelles, implémentations de référence en Python ou TypeScript, analyses de complexité et critères de succès mesurables.",
  },

  // ===== TESTIMONIALS =====
  'testimonials.kicker': { en: '// Testimonials', fr: '// Témoignages' },
  'testimonials.title': {
    en: 'Trusted by digital teams and creators worldwide.',
    fr: "La confiance d'équipes digitales et de créateurs dans le monde entier.",
  },
  'testi.1': {
    en: 'Gregory rebuilt our entire AI content process from the ground up. What used to take our editorial team three days now ships in four hours, with significantly higher consistency. His optimisation of our prompt pipeline alone unlocked a measurable jump in output quality.',
    fr: "Gregory a reconstruit l'intégralité de notre process de contenu IA depuis zéro. Ce qui prenait à notre équipe éditoriale trois jours est livré en quatre heures, avec une cohérence nettement supérieure. L'optimisation de notre pipeline de prompts a, à elle seule, débloqué un saut mesurable de qualité.",
  },
  'testi.2': {
    en: 'We hired Gregory to architect a custom interactive software tool for our online programs. The clarity of his digital toolset, the documentation and the underlying data model are unlike anything we had encountered before. A genuine engineering mindset applied to digital content.',
    fr: "Nous avons fait appel à Gregory pour concevoir un logiciel interactif sur-mesure pour nos programmes en ligne. La clarté de ses outils numériques, la documentation et le modèle de données sous-jacent dépassent tout ce que nous avions rencontré. Un véritable état d'esprit d'ingénieur appliqué au contenu digital.",
  },
  'testi.3': {
    en: "The efficiency of Gregory's logical models transformed how our team approaches AI features. He reframed problems we had been wrestling with for months into clean algorithmic statements — and shipped working prototypes in days. Highly recommended for any serious digital team.",
    fr: "L'efficacité des modèles logiques de Gregory a transformé l'approche de notre équipe sur les fonctionnalités IA. Il a reformulé en énoncés algorithmiques propres des problèmes sur lesquels nous tournions depuis des mois — et a livré des prototypes fonctionnels en quelques jours. Hautement recommandé pour toute équipe digitale sérieuse.",
  },

  // ===== CTA SECTION =====
  'cta.title': {
    en: 'Ready to engineer your next AI workflow?',
    fr: 'Prêt à concevoir votre prochain workflow IA ?',
  },
  'cta.subtitle': {
    en: 'Send a brief description of your project. Typical response time is under 48 hours.',
    fr: 'Envoyez une description brève de votre projet. Délai de réponse habituel : moins de 48h.',
  },

  // ===== FOOTER =====
  'footer.tagline': {
    en: 'Full-Stack Software Engineer building AI systems and Digital Content Development platforms. Operating internationally as an Independent Contractor.',
    fr: "Full-Stack Software Engineer construisant des systèmes IA et plateformes de Digital Content Development. Opérant à l'international en tant qu'Independent Contractor.",
  },
  'footer.navigate': { en: 'Navigate', fr: 'Navigation' },
  'footer.servicesCol': { en: 'Services', fr: 'Services' },
  'footer.built': {
    en: 'Built with engineering rigor · EN / FR',
    fr: "Conçu avec rigueur d'ingénieur · EN / FR",
  },

  // ===== SERVICES PAGE =====
  'svc.page.kicker': { en: '// Services', fr: '// Services' },
  'svc.page.title': {
    en: 'Full-stack software engineering for AI-driven organisations.',
    fr: "Software engineering full-stack pour organisations AI-driven.",
  },
  'svc.page.subtitle': {
    en: 'Three services delivered as a Full-Stack Software Engineer and Independent Contractor — under a formal scope of work, with versioned deliverables, evaluation criteria and full transfer of ownership at handover.',
    fr: "Trois prestations délivrées en tant que Full-Stack Software Engineer et Independent Contractor — sous périmètre formel, avec livrables versionnés, critères d'évaluation et transfert de propriété intégral à la livraison.",
  },
  'svc.deliverables': { en: 'Deliverables', fr: 'Livrables' },
  'svc.capabilities': { en: 'Capabilities', fr: 'Compétences' },
  'svc.engagement': { en: 'Typical engagement', fr: 'Mission type' },

  'svc.ai.long': {
    en: 'I design, build and ship full-stack AI features: back-end APIs around LLM calls, typed prompt pipelines with evaluation harnesses, retrieval-augmented systems with vector databases, observability and CI/CD. From single-prompt utilities to multi-agent orchestrations — engineered to survive production traffic.',
    fr: "Je conçois, construis et livre des features IA full-stack : back-ends API autour des appels LLM, pipelines de prompts typés avec harnais d'évaluation, systèmes RAG avec vector databases, observabilité et CI/CD. Du prompt unitaire à l'orchestration multi-agents — pensé pour survivre au trafic de production.",
  },
  'svc.content.long': {
    en: 'I engineer the full-stack software platforms behind your Digital Content Development: interactive web apps in React and TypeScript, back-end APIs in Python or Node, structured content data models, custom CMSes, production pipelines and Digital Creative Content delivery. Proprietary software, end-to-end, you own the source code.',
    fr: "J'ingénieurise les plateformes software full-stack derrière votre Digital Content Development : web apps interactives en React et TypeScript, back-ends API en Python ou Node, modèles de données structurés, CMS sur-mesure, pipelines de production et livraison de Digital Creative Content. Du logiciel propriétaire de bout en bout, dont vous possédez le code source.",
  },
  'svc.logic.long': {
    en: 'Algorithmic engineering for software teams. I translate ambiguous product questions into formal algorithmic specifications, reference implementations in Python or TypeScript, complexity analysis and benchmark harnesses — bridging mathematical reasoning and shippable software.',
    fr: "Ingénierie algorithmique pour équipes software. Je transforme des questions produit ambiguës en spécifications algorithmiques formelles, implémentations de référence en Python ou TypeScript, analyses de complexité et harnais de benchmark — entre raisonnement mathématique et software livrable.",
  },

  // Process
  'process.kicker': { en: '// How we work', fr: '// Méthode de travail' },
  'process.title': { en: 'A predictable engagement process.', fr: 'Un processus de mission prévisible.' },
  'process.s1.title': { en: 'Discovery call', fr: 'Appel discovery' },
  'process.s1.desc': { en: '30-min call to scope the problem, constraints and success criteria.', fr: 'Call de 30 min pour cadrer le problème, les contraintes et les critères de succès.' },
  'process.s2.title': { en: 'Written proposal', fr: 'Proposition écrite' },
  'process.s2.desc': { en: 'Formal scope, deliverables, timeline, pricing and evaluation criteria — within 5 business days.', fr: 'Périmètre formel, livrables, calendrier, tarif et critères d\'évaluation — sous 5 jours ouvrés.' },
  'process.s3.title': { en: 'Build & iterate', fr: 'Build & itération' },
  'process.s3.desc': { en: 'Weekly written status, async-first collaboration, scheduled review calls.', fr: 'Status hebdo écrit, collaboration async-first, calls de revue planifiés.' },
  'process.s4.title': { en: 'Handover', fr: 'Transfert' },
  'process.s4.desc': { en: 'Source code, documentation, training and operational playbook handed over in full.', fr: 'Code source, documentation, formation et playbook opérationnel transférés intégralement.' },

  // ===== ABOUT PAGE =====
  'about.kicker': { en: '// About', fr: '// À propos' },
  'about.title': {
    en: 'Full-Stack Software Engineer. Mathematical mind. Senior partner for AI and content platforms.',
    fr: 'Full-Stack Software Engineer. Esprit mathématique. Partenaire senior pour vos plateformes IA et contenu.',
  },
  'about.p1': {
    en: "I'm a Full-Stack Software Engineer with a decade of production experience. I write code on both ends of the stack — type-safe front-ends in React and TypeScript, back-ends in Python and Node, API design, databases, infrastructure, CI/CD and observability — for organisations integrating Artificial Intelligence into real products and shipping Digital Content Development at scale.",
    fr: "Je suis Full-Stack Software Engineer avec dix ans d'expérience en production. J'écris du code des deux côtés de la stack — front-ends typés en React et TypeScript, back-ends en Python et Node, design d'API, bases de données, infrastructure, CI/CD et observabilité — pour des organisations intégrant de l'Intelligence Artificielle dans de vrais produits et livrant du Digital Content Development à l'échelle.",
  },
  'about.p2': {
    en: 'My background in advanced mathematics and algorithmic reasoning is the engine behind every engagement. It is what allows me to formalise vague product questions into measurable systems, to evaluate LLM behavior with rigor instead of intuition, to write Solidity smart contracts that hold under adversarial conditions, and to design interactive software that scales without collapsing into entropy.',
    fr: "Mon background en mathématiques avancées et raisonnement algorithmique est le moteur de chaque mission. C'est ce qui me permet de formaliser des questions produit vagues en systèmes mesurables, d'évaluer le comportement d'un LLM avec rigueur plutôt qu'à l'intuition, d'écrire des smart contracts Solidity qui tiennent en conditions adversariales, et de concevoir des logiciels interactifs qui scalent sans s'effondrer dans l'entropie.",
  },
  'about.p3': {
    en: 'I operate exclusively as an Independent Contractor — a senior, hands-on Full-Stack Software Engineer for founders, product teams and digital content studios that need a partner, not a generalist. Every engagement is scoped in writing, delivered transparently with versioned code on a shared Git repository, and concluded with full transfer of ownership at handover.',
    fr: "J'opère exclusivement en tant qu'Independent Contractor — un Full-Stack Software Engineer senior et opérationnel, pour des fondateurs, équipes produit et studios de contenu digital qui ont besoin d'un partenaire, pas d'un généraliste. Chaque mission est cadrée par écrit, livrée en toute transparence avec du code versionné sur un dépôt Git partagé, et conclue par un transfert de propriété intégral.",
  },

  'method.kicker': { en: '// Methodology', fr: '// Méthodologie' },
  'method.title': { en: 'Four principles that govern every engagement.', fr: 'Quatre principes qui gouvernent chaque mission.' },
  'method.p1.title': { en: 'Specification-first', fr: "Spécification d'abord" },
  'method.p1.desc': { en: 'Every workflow begins with a formal contract: inputs, outputs, edge cases, evaluation criteria. No ambiguity, no scope creep.', fr: 'Chaque workflow commence par un contrat formel : entrées, sorties, cas limites, critères d\'évaluation. Aucune ambiguïté, aucun scope creep.' },
  'method.p2.title': { en: 'Measure, then iterate', fr: 'Mesurer, puis itérer' },
  'method.p2.desc': { en: 'Quantitative evaluation harnesses replace gut-feel iteration on prompts and pipelines. Every change is justified by data.', fr: 'Des harnais d\'évaluation quantitatifs remplacent l\'itération au feeling sur les prompts et pipelines. Chaque changement est justifié par la donnée.' },
  'method.p3.title': { en: 'Transfer ownership', fr: 'Transférer la propriété' },
  'method.p3.desc': { en: 'Clients leave each engagement with documentation, training and full control of their stack. No lock-in, no opacity.', fr: 'Les clients repartent de chaque mission avec documentation, formation et contrôle total de leur stack. Aucun lock-in, aucune opacité.' },
  'method.p4.title': { en: 'Async-first', fr: 'Async-first' },
  'method.p4.desc': { en: 'Weekly written reviews, structured async updates, scheduled deep-work blocks. Designed for international multi-timezone collaboration.', fr: 'Revues écrites hebdomadaires, updates async structurés, blocs de deep work planifiés. Conçu pour la collaboration internationale multi-fuseaux.' },

  'stack.title': { en: 'Tools and frameworks I work with.', fr: 'Outils et frameworks utilisés.' },

  // ===== CASE STUDIES =====
  'cs.kicker': { en: '// Case Studies', fr: '// Études de cas' },
  'cs.title': {
    en: 'Selected engagements. Anonymised when required by contract.',
    fr: "Missions sélectionnées. Anonymisées lorsque le contrat l'exige.",
  },
  'cs.subtitle': {
    en: 'A representative sample of work delivered as an Independent Contractor across AI Optimization, Digital Content Development and applied algorithmic consulting.',
    fr: "Échantillon représentatif de missions livrées en tant qu'Independent Contractor en AI Optimization, Digital Content Development et conseil algorithmique appliqué.",
  },
  'cs.challenge': { en: 'Challenge', fr: 'Problématique' },
  'cs.approach': { en: 'Approach', fr: 'Approche' },
  'cs.outcome': { en: 'Outcome', fr: 'Résultat' },

  // ===== CONTACT =====
  'contact.kicker': { en: '// Contact', fr: '// Contact' },
  'contact.title': { en: "Let's design your next AI engagement.", fr: 'Concevons votre prochaine mission IA.' },
  'contact.subtitle': {
    en: 'I work as an Independent Contractor with international clients, in English and French. Send a brief description of your project — typical response time is under 48 hours.',
    fr: "Je travaille en tant qu'Independent Contractor avec des clients internationaux, en anglais et en français. Envoyez une brève description de votre projet — délai de réponse habituel : moins de 48h.",
  },
  'contact.form.name': { en: 'Your full name', fr: 'Votre nom complet' },
  'contact.form.company': { en: 'Company', fr: 'Entreprise' },
  'contact.form.budget': { en: 'Budget range (USD)', fr: 'Budget (USD)' },
  'contact.form.type': { en: 'Engagement type', fr: 'Type de mission' },
  'contact.form.brief': { en: 'Project brief', fr: 'Brief du projet' },
  'contact.form.recipient': { en: 'Recipient (Independent Contractor)', fr: 'Destinataire (Independent Contractor)' },
  'contact.form.placeholder.recipient': {
    en: 'Gregory Mittelette',
    fr: 'Gregory Mittelette',
  },
  'contact.form.recipientHint': {
    en: '↑ Replace this placeholder with your full legal professional name before publishing.',
    fr: '↑ Remplacez ce placeholder par votre nom légal professionnel complet avant publication.',
  },
  'contact.form.submit': { en: 'Send project brief', fr: 'Envoyer le brief' },
  'contact.form.success': {
    en: "Thank you — your brief has been queued. You'll receive a reply within 48 hours.",
    fr: 'Merci — votre brief a bien été envoyé. Vous recevrez une réponse sous 48h.',
  },
  'contact.faq': { en: 'FAQ', fr: 'FAQ' },

  // ===== LEGAL =====
  'legal.kicker': { en: '// Legal', fr: '// Mentions légales' },
  'legal.title': {
    en: 'Legal notice & Independent Contractor status',
    fr: 'Mentions légales & statut Independent Contractor',
  },
} as const;

export type TKey = keyof typeof translations;
